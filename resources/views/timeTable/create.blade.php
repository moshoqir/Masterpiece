                    ...
                    <form method="POST" action="{{ route('timetable.store') }}">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control @error('day') is-invalid @enderror" id="day" name="day" required>
                                <option aria-readonly="true" value="">Select a day</option>
                                @foreach($days as $day)
                                    <option value="{{ $day }}" {{ old('day', request('day')) == $day ? 'selected' : '' }}>{{ $day }}</option>
                                @endforeach
                            </select>
                            @error('day')
                                <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control @error('time_slot') is-invalid @enderror" id="time_slot" name="time_slot" required>
                                ...
                                @foreach($timeSlots as $timeSlot)
                                    <option value="{{ $timeSlot }}" {{ old('time_slot', request('time_slot')) == $timeSlot ? 'selected' : '' }}>{{ $timeSlot }}</option>
                                @endforeach
                            </select>
                            @error('time_slot')
                                <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="form-check mb-3">
                            <input type="checkbox" class="form-check-input" id="dark_bg" name="dark_bg" value="1" {{ old('dark_bg') ? 'checked' : '' }}>
